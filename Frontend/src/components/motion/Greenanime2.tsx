...
import { cyberAudio, cyberMusic } from "@/lib/cyberAudio";
...
export default function GreenAnime({ onComplete }: GreenAnimeProps = {}) {
  ...
  const containmentCircleRef = useRef<SVGCircleElement>(null);
  const containmentHexRef = useRef<SVGPolygonElement>(null);
  const containmentSpokesRef = useRef<(SVGLineElement | null)[]>([]);
    ...
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        onUpdate: () => {
          ...
          cyberMusic.updateTimeline(t, 25.0);
          cyberMusic.transitionToFrame(currentFrame, t);
        },
        onComplete: () => {
          cyberMusic.stopScore(0.38);
          gsap.to(rootRef.current, {
            opacity: 0,
            duration: 0.38,
            ease: "power2.inOut",
      ...
      if (containmentCircleRef.current) {
        gsap.set(containmentCircleRef.current, {
          opacity: 0,
          attr: { r: 112 },
        });
      }

      if (containmentHexRef.current) {
        gsap.set(containmentHexRef.current, {
          opacity: 0,
          attr: {
            points: STONES.map((stone) => {
              const p = point(stone.angle, 112);
              return `${p.x},${p.y}`;
            }).join(" "),
          },
        });
      ...
      gsap.set(containmentSpokesRef.current.filter(Boolean), {
        opacity: 0,
      });
       ...
       * SIX-WAY MECHANICAL CONTAINMENT
       *
       * Locks are integrated directly inside each stone's group.
       * In Frame 09 they slide in along their radial axis to clamp the stone.
       */
      /*
       * FRAME 09 — SIX-WAY ALLOCATION / LOCK DEPLOYMENT
       * ---------------------------------------------------------------
       * Restore the physical six-lock allocation animation.
       *
       * Each lock has its OWN radial lane:
       *
       *        [R] 
       *          ↓
       * [S]  →  CORE  ←  [M]
       ...
       *        [T]
       *
       * Because each mechanical lock is already inside its stone group,
       * the local approach vector below is rotated automatically by that
       * stone's six-way parent rotation.  This keeps every lock aligned
       * with its own stone instead of making all six use one screen-space
       * trajectory.
       */
      const allocationStartX = 330;
      const allocationStartY = 225;

      structureRefs.current.forEach((structure, i) => {
        if (!structure) return;

        // Deliberately stagger the six allocations so they feel like
        // six independent containment mechanisms locking in sequence.
        const t = 10.20 + i * 0.17;
        ...
        // 1. Wake the mechanism at its assigned outer position.
        tl.to(structure, {
          opacity: 0.25,
          scale: 0.64,
          duration: 0.14,
        ...
        // 2. Main allocation: travel along this lock's own radial rail.
        tl.to(structure, {
          opacity: 1,
          x: 115,
          y: 78,
        ...
        // 4. Hard mechanical seat directly onto the stone.
        tl.to(structure, {
          x: 0,
          y: 0,
          scale: 0.72,
        ...
        // 5. Tiny recoil proves that the mechanism actually locked.
        tl.to(structure, {
          x: -5,
          y: -3,
          scale: 0.75,
        ...
        // Each lock gets its own impact at the exact seating moment.
        tl.call(() => {
          impact([18, 14, 22], "lock");
        }, [], t + 0.80);
      });

      // Six-way allocation pulse: the containment shell responds as
      // each individual mechanism reaches its assigned node.
      STONES.forEach((stone, i) => {
        const t = 10.20 + i * 0.17 + 0.80;

        tl.to(containmentCircleRef.current, {
          attr: { r: 118 + i * 1.5 },
          opacity: 0.52,
          duration: 0.08,
          ease: "power2.out",
        ...
        tl.to(containmentHexRef.current, {
          scale: 1.045,
          transformOrigin: `${CX}px ${CY}px`,
          duration: 0.08,
          yoyo: true,
      ...
      /*
       * Six independent energy feeds now launch immediately after their
       * corresponding mechanical allocation.  The feed and lock therefore
       * read as one physical containment action.
       */
      ...
      /* SIX-WAY CONTAINMENT — fixed axes, center-origin pulse. */
      tl.to(
        [containmentCircleRef.current, containmentHexRef.current],
        {
          opacity: 1,
          duration: 0.42,
          ease: "power2.out",
      ...
      tl.to(containmentSpokesRef.current.filter(Boolean), {
        opacity: 1,
        duration: 0.30,
        stagger: 0.025,
        ease: "power2.out",
      ...
      const updateContinuousLoad = (value: number) => {
        ...
        STONES.forEach((stone, i) => {
          ...
          /*
           * 3. LOCK CONTAINMENT (IN THE SAME ENLARGING GROUP)
           *
           * The structure is a child of stoneEl, so it inherits the exact
           * same nodeX, nodeY translation and radial expansion automatically.
           */
          const structure = structureRefs.current[i];
          if (structure) {
            // Allocation owns x/y. Overload only enlarges the already
            // seated mechanism so it remains physically attached to the
            // matching six-way stone.
            gsap.set(structure, {
              scale: 0.72 + expansion * 0.08,
            });
          }
        });
        ...
        /*
         * SIX-WAY CONTAINMENT — SAME EXPANSION AS THE REACTOR
         *
         * The old implementation scaled the whole network as a separate
         * object. That made the circle/hexagon lag behind the core and the
         * six stones. Now their actual SVG geometry is recalculated from
         * the same center and expansion value.
         */
        const containmentRadius = 112 + expansion * 86;

        if (containmentCircleRef.current) {
          gsap.set(containmentCircleRef.current, {
            attr: { r: containmentRadius },
            opacity: 0.30 + expansion * 0.42,
          });
        }

        if (containmentHexRef.current) {
          const hexPoints = STONES.map((stone) => {
            const p = point(stone.angle, containmentRadius);
            return `${p.x},${p.y}`;
          }).join(" ");

          gsap.set(containmentHexRef.current, {
            attr: { points: hexPoints },
            opacity: 0.34 + expansion * 0.52,
            strokeWidth: 2 + expansion * 2.2,
          });
        }

        containmentSpokesRef.current.forEach((spoke, i) => {
          if (!spoke) return;

          const stone = STONES[i];
          const inner = point(stone.angle, 54 + expansion * 22);
          const outer = point(stone.angle, containmentRadius);
          ...
          gsap.set(spoke, {
            ...
            strokeWidth: 1.2 + expansion * 1.8,
            strokeOpacity: 0.28 + expansion * 0.42,
          });
        });
        ...
        // Keep the containment shell visually locked to the same center.
        if (containmentHexRef.current) {
          gsap.set(containmentHexRef.current, {
            transformOrigin: `${CX}px ${CY}px`,
          });
      ...
      >
        ...
        >
          ...
          {/* =========================================================
              SIX-WAY CONTAINMENT
              ---------------------------------------------------------
              ONE central geometry:
                ENERGY CORE -> SIX-SIDED HEXAGON -> CIRCLE -> SIX AXES

              The hexagon and circle are driven from the exact same
              center and expansion value as the six stone channels.
              ========================================================= */}
          <g ref={networkRef}>
            <polygon
              ref={containmentHexRef}
              points={STONES.map((stone) => {
                const p = point(stone.angle, 112);
                return `${p.x},${p.y}`;
              }).join(" ")}
              fill="rgba(52,211,153,.035)"
              stroke="#86efac"
              strokeOpacity=".34"
              strokeWidth="2"
              strokeDasharray="8 8"
            ...
            {STONES.map((stone, i) => {
              const inner = point(stone.angle, 54);
              const outer = point(stone.angle, 112);

              return (
                <g key={`containment-axis-${stone.id}`}>
                  <line
                    ref={(el) => {
                      containmentSpokesRef.current[i] = el;
                    }}
                    x1={inner.x}
                    y1={inner.y}
                    x2={outer.x}
          ...
          {/* SIX COLORED FRAGMENTS + INTEGRATED MECHANICAL CONTAINMENT LOCKS */}
          {STONES.map((stone, i) => {
            return (
              <g
                key={stone.id}
              ...
              >
                {/* MECHANICAL CONTAINMENT LOCK (IN THE SAME ENLARGING GROUP) */}
                <g transform={`rotate(${stone.angle - 33.69})`}>
                  <g
                    ref={(el) => {
                      structureRefs.current[i] = el;
                    }}
                  >
                  {/* MECHANICAL BODY / STRING */}
                  <path
        ...
        <div className="absolute right-[3%] top-[3%] z-[10000] flex items-center gap-2">
          ...
          <button
            ...
            className="flex cursor-pointer items-center gap-2 rounded border border-emerald-500/20 bg-black/60 px-3 py-2 font-mono text-[9px] tracking-[0.2em] text-emerald-400 backdrop-blur-sm transition-all hover:border-emerald-400 hover:bg-emerald-950/40"
            title="Toggle Procedural Cyber Audio (M)"
          >
            <span
          ...
          <button
            type="button"
            onClick={handleSkip}
            className="group flex cursor-pointer items-center gap-2 rounded border border-emerald-500/20 bg-black/60 px-3.5 py-2 font-mono text-[9px] tracking-[0.24em] text-emerald-500/80 backdrop-blur-sm transition-all hover:border-emerald-400 hover:bg-emerald-950/40 hover:text-emerald-400"
          >
